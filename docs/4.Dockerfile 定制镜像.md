...
$ docker history nginx:v3  #查看镜像结构
$ docker run --name webserv3 -d -p 8083:80 nginx:v3 #验证构建的镜像
$ docker ps #查看镜像
```
...
```shell
$ docker build -t nginx:v3 .
Sending build context to Docker daemon 2.048 kB  #注意：发送到Docker daemon构建
...
```
